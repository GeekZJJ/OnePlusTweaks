package com.geekzjj.oneplustweaks;

public class Common {
    public static String PACKAGE_ONEPLUS_SCREENSHOT = "com.oneplus.screenshot";
    public static String PACKAGE_ONEPLUS_DIALER = "com.android.dialer";
    public static String PACKAGE_ONEPLUS_MMS = "com.android.mms";
    public static String PACKAGE_ONEPLUS_DESKCLOCK = "com.oneplus.deskclock";
    public static String PACKAGE_ONEPLUS_CONFIG = "com.oneplus.config";
    public static String PACKAGE_ANDROID = "android";
    public static String PACKAGE_SYSTEMUI = "com.android.systemui";
    public static String PACKAGE_ANCROID_PHONE = "com.android.phone";

    public static final String ACTION_SETTINGS_CHANGED = "action_settings_changed";

    public static final String KEY_VOLUME_EXPAND_STREAMS = "volume_panel_items";
    public static final String KEY_VOLUME_PANEL_EXPANDED = "volume_panel_expanded";
    public static final String KEY_VOLUME_PANEL_LOCATION = "volume_panel_location";

    public static final String KEY_HOTSPOT_HIDDEN_SSID = "hotspot_hidden_ssid";
    public static final String KEY_HOTSPOT_2G_CHANNEL = "hotspot_2G_channel";
    public static final String KEY_HOTSPOT_5G_CHANNEL = "hotspot_5G_channel";

    public static final String KEY_RECENTS_ADD_CLEAR_BTN = "recents_add_clear_button";
    public static final String KEY_CLEAR_RECENTS_LOCATION = "recents_clear_button_location";

    public static final boolean DEFAULT_VOLUME_PANEL_EXPANDED = true;
    public static final String[] DEFAULT_VOLUME_EXPAND_STREAMS = {"3", "2", "4","0"};
    public static final int DEFAULT_VOLUME_PANEL_LOCATION = 1;

    public static final int DEFAULT_HOTSPOT_2G_CHANNEL = 0;
    public static final int DEFAULT_HOTSPOT_5G_CHANNEL = 44;
    public static final boolean DEFAULT_HOTSPOT_HIDDEN_SSID = false;

    public static final boolean DEFAULT_RECENTS_ADD_CLEAR_BTN = true;
    public static final int DEFAULT_CLEAR_RECENTS_LOCATION = 5;
}
